...
You will then receive instructions and a URL for signing up an API key. You will obtain a API key immediately, but it is not ready for use immediately. You will need to send the key back to the Regulations.gov Help Desk for the developers to activate your key for use on Regulations.gov. The activation process may take a week or 10 days.
